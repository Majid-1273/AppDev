...
const Batches = ({ navigation, batches }) => {
  const [searchText, setSearchText] = useState('');
  const [filter, setFilter] = useState('All');
  const [filteredBatches, setFilteredBatches] = useState(batches);

  // Static images based on batch type (same as in Batch.js)
  ...
  const handleDeleteBatch = async (batchId) => {
    ...
    Alert.alert(
      'Confirm Delete',
      'Are you sure you want to delete this batch? This will also delete all related feed, egg production, mortality, and vaccination records.',
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              // Delete the main batch document from the batches collection
              await deleteDoc(doc(db, 'batches', batchId));

              // Delete all related records from other collections
              await deleteRelatedRecords(batchId);

              Alert.alert('Success', 'Batch and all related records deleted successfully');
  ...
  // Helper function to delete all related records
  const deleteRelatedRecords = async (batchId) => {
    const collections = ['feed', 'egg-production', 'mortality', 'vaccination'];
    
    for (const collectionName of collections) {
      try {
        // Query for documents with matching batchId
        const q = query(
        ...
        console.log(`Deleted ${querySnapshot.docs.length} records from ${collectionName}`);
      } catch (error) {
  ...
  const renderItem = ({ item }) => (
    ...
    >
      <Image 
        source={{ uri: getBatchImage(item.type) }} 
        style={styles.batchesImage}
        defaultSource={{ uri: 'https://images.unsplash.com/photo-1612170153139-6f881ff067e0?w=400&h=300&fit=crop&crop=center' }}
      />
      <View style={styles.batchInfo}>
        <Text style={styles.batchesName}>{item.name}</Text>
        <Text style={styles.batchDetails}>
          {item.type} • {item.currentCount || item.initialCount || 0} birds
        </Text>
        <Text style={styles.batchAge}>
          Age: {item.age || calculateAge(item.placementDate)}
        </Text>
      </View>
      <View style={styles.statusIndicator}>
        <View style={[styles.statusDot, { backgroundColor: getStatusColor(item.healthStatus) }]} />
        <Text style={styles.statusText}>{item.healthStatus || 'Healthy'}</Text>
      </View>
    </TouchableOpacity>
  );
  ...
  const calculateAge = (placementDate) => {
    ...
    try {
      const now = new Date();
      const placement = placementDate.toDate ? placementDate.toDate() : new Date(placementDate);
  ...
  return (
    <View style={styles.batchesContainer}>
      <Text style={styles.screenTitle}>My Batches</Text>
